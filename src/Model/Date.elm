...
monthsBetween (Date dA) (Date dB) =
    case (dA.month, dB.month) of
        (Just mA, Just mB) ->
            Just <| if dA.year == dB.year
                then abs (monthsBetweenMonths mA mB)
                else if dA.year < dB.year
                    then ( dB.year - dA.year ) * 12 + (monthsBetweenMonths Jan mB) - (monthsBetweenMonths Jan mA)
                    else ( dA.year - dB.year ) * 12 + (monthsBetweenMonths Jan mA) - (monthsBetweenMonths Jan mB)
...
view : Date -> Html msg
view (Date d) =
    div [] [ text <| String.fromInt d.year ++ " " ++ Maybe.withDefault "" (Maybe.map monthToString d.month) ]
